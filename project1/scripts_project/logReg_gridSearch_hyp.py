...
class GridSearch_Simulation:
    ...
    def get_data_and_params(self):
        ...
        #add the constants at the beginning
        ...
        #####################################################################
        l_rates = np.logspace(-5, 0, 7)
    

        ####################################################################
        # Choose the batch sizes
        ##################################################################
        b_sizes = np.array([10000, 20000, 50000, -1])

        
        ################################################################
        # Choose initializations
        ###################################################################
        initial_w_distributions = ['zero', "normal"]
    ...
    def train_model(self, dataset_idx, l_rate, b_size, initial_w_dist):
        ...
        for k in range(k_fold):
            ...
            '''print('Check shape train', x_train_k.shape)
            print('Check shape test', x_test_k.shape)

            print('Check standardization')
            print('train')
            print(x_train_k.mean(axis = 0))
            print(x_train_k.std(axis = 0))

            print('test')
            print(x_test_k.mean(axis = 0))
            print(x_test_k.std(axis = 0))
'''
...
if __name__ == '__main__':
    ...
    with concurrent.futures.ProcessPoolExecutor() as executor:
        ...
        print(res_list)
        print(len(res_list))
        with open('../results/logreg_gridsearch_hyp.txt', 'wb') as fh:
